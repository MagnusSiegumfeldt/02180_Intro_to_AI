        ...
        while len(stack) > 0:
            ...
            for dr, dc in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                ...
                if 0 <= nr < 9 and 0 <= nc < 9 and not visited[nr][nc] and gamestate.board[nr][nc] == color: 
                    stack.append((nr, nc))
                    visited[nr][nc] = True
                elif 0 <= nr < 9 and 0 <= nc < 9 and not counted[nr][nc] and gamestate.board[nr][nc] == 0:
                    boundary += 1
                    counted[nr][nc] = True
    ...
    def eval(gamestate): 
        ...
        for i in range(9):
            for j in range(9):
                ...
                size, boundary = MinimaxAreaAdvanced.dfs(gamestate, i, j, visited, counted, current_color)
