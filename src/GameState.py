                ...
                if self.board[row_white][col_white] == 0:
                    ...
                    for dr, dc in directions:
                        ...
                        if (
                            ...
                            friendly_row, friendly_col = (
                                (row_white, col_white)
                                if current_player == 1
                                else (row_black, col_black)
                            )
                            opponent_row, opponent_col = (
                                (row_black, col_black)
                                if current_player == 1
                                else (row_white, col_white)
                            )

                            friendly_has_friendly_neighbors = (
                                self.has_friendly_neighbors(
                                    friendly_row, friendly_col, current_player
                                )
                            )
                            opponent_has_friendly_neighbors = (
                                self.has_friendly_neighbors(
                                    opponent_row, opponent_col, opponent_player
                                )
                            )
